...
char myHash = 90;
int elbow_pos = 135;
double wristTheta_dest = 0;
double wristPhi_dest = 0;
int clawL_pos = 75;
int clawR_pos = 150;
        ...
        if (incomingByte == -127 && !inTransmission) {
            ...
            switch (bytesRead) {
                ...
                case 7: // hash (last byte recieved in transmission)
                    ...
                    inTransmission = false;
                    myHash = (shoulder_pos + elbow_pos + uint8_t(wristTheta_dest) + long(wristPhi_dest) + clawL_pos + clawR_pos) / 6;
                    if (myHash == serialHash) {
                        sprintf(serResp, "%d\t%d\t%d\t%d\t%d\t%d\t%d", shoulder_pos, elbow_pos, (uint8_t)wristTheta_dest,
                                (long)wristPhi_dest, clawL_pos, clawR_pos, myHash);
                        Serial.println(serResp);
                        updateServos();
                    } else {
                        sprintf(serResp, "%d\t%d\t%d\t%d\t%d\t%d\t%d!!!!!!!", shoulder_pos, elbow_pos, (uint8_t)wristTheta_dest, (long)wristPhi_dest, clawL_pos, clawR_pos, myHash);
                        Serial.println(serResp);
                    }
                    break;
            }
